...
The `DyteMeetingHandler` instance (referenced as `meeting` above) helps you interact with the meeting. You can take actions on the meeting as well as the participants. You can also setup custom controls to that interact with the meeting in certain ways that help you send events and messages across to other participants in the meeting. You can find the complete reference to the `DyteMeetingHandler` type [here](./reference/meeting). Read on to understand how to use these individual controls.
