...
## Turn off the setup / preview screen

The audio / video setup screen that appears for the participant right before getting into the meeting is turned on by default. You can set `showSetupScreen` in `meetingConfig` to `fakse` to turn it off (default value is `true`).
...
## Style Isolation - ShadowDOM [Experimental]

If your app's css is being affected by our css or vice versa, you can enable the experimental ShadowDOM modern
...
This will render Dyte UI under a shadow DOM isolating your app's and our SDKs styling


## Set default camera preference

Choose if you want the default camera to be front or back for mobile devices

Use `environment` for back camera. Defaults to `user` which is the front camera
