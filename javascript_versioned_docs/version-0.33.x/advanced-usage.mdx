...
The audio / video setup screen that appears for the participant right before getting into the meeting is turned on by default. You can set `showSetupScreen` in `meetingConfig` to `false` to turn it off (default value is `true`).
