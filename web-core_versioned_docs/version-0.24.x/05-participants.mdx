...
You can subscribe to events for all participants using `meeeting.participants.on()` method.
...
```ts
meeting.participants.on('viewModeChanged', ({ viewMode, currentPage, pageCount }) => {
...
```ts
meeting.participants.on('pageChanged', ({ viewMode, currentPage, pageCount }) => {
