...
the initial values for these options are by degault set to ease the process on integration. 
You can override them at the client end based on specific use cases or events.
...
```
- `textColor`: Your primary text color on brand color and text color on background.
```kotlin
text = TextColor(
    onBrand = TextColor.TextColorOnBrand(
        shade1000 = Color.parseColor("#ff002b4c"),
