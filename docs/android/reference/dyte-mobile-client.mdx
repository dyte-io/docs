...
`DyteMobileClient` is the main core class of UI Kit sdk. This class has the actual businees api's required to join in a meeting, send chat message, create polls, etc. While using UI Kit one can also listen to all the important events in a meeting with help of this class. It mainly exposes 3 types of listeners. Meeting events, Participant events, Self events. 
