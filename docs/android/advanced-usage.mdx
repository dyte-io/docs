...
val config = DyteUIKitConfig(
  ...
  designTokens = getImarticusTheme()
)
...
You can get the UI configuration of the meeting in progress using the `designTokens`
property of `Meeting`.
...
You can take actions on the meeting as well as the participants.
...
```kotlin
val particiapnt = meeting.participants.joined.find { it.id = "my_participant_id" }
```
...
```kotlin
val video = particiapnt.videoEnabled;
```
...
```kotlin
val videoStream = particiapnt.getVideoView()
```
...
```kotlin
val pinned = particiapnt.isPinned;
```
