...
Each participant in each of the `joined`, `waitlisted`, `active`, and `pinned` maps is of type [`DyteParticipant`](../reference/DyteParticipant.md). Read more about each individual `participant` object [here](../participants/participant-object.mdx).
