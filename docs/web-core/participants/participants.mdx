---
sidebar_position: 1
slug: /participants/participants
...
Each participant in each of the `joined`, `waitlisted`, `active`, and `pinned` maps is of type [`DyteParticipant`](../reference/DyteParticipant.md). Read more about each individual `participant` object [here](./participant-object).
