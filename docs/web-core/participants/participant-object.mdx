...
meeting.participants.joined.get(participantId).on('audioUpdate', ({ audioEnabled, audioTrack }) => {
  ...
  console.log('The participant with id', participantId, 'has toggled their mic to', audioEnabled);
});
```
...
Read more about the participant events in the [events](../reference/DyteParticipant.md) section in the API reference.
