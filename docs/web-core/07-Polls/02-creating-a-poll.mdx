...
slug: creating-a-poll
tags: [web-core, polls, create]
---

# Creating a poll

A new poll can be created using the `create` method from the [`meeting.polls`](./introduction) object.

The `meeting.polls.create()` method accepts the following params and returns `Promise<void>`.
...
The following code creates a poll where votes are anonymous.
