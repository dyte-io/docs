...
### Detecting when user toggles their audio

This event is triggered when the user mutes / unmutes their audio

```javascript
meeting.on(meeting.Events.audioUpdated, (audioEnabled) => {});
```

### Detecting when user toggles their video

This event is triggered when the user toggles their video off or on

```javascript
meeting.on(meeting.Events.videoUpdated, (videoEnabled) => {});
```

### Detecting when user screenshares

This event is triggered when the user starts or stops screensharing
