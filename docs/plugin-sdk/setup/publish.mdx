...
Once you've created a plugin, you need to publish it in order to use it during
the meeting. You can publish multiple versions of your plugin.
...
4.  you can use the `--no-deploy` flag if you wish to only publish the plugin
    and not deploy it.

```shell
dyte plugins publish --no-deploy
```
