...
This method initialises the SDK and returns a plugin object. The Client SDK (web/mobile) receives a `ready` event once the initialisation is complete.
...
### Initialise without data exchange

This method initialises the SDK without emitting a `ready` event to the client SDK.
The user can choose to manually emit this later when the plugin is ready to start event/data exchange with client SDK.
