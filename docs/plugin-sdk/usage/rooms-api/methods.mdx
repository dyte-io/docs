...
### Get Peer
:::info note
Returns `undefined` if the peer with given `ID` has left the meeting or an incorrect `ID` is provided.
:::
...
:::tip
You can current peer's (self) data by not passing an `ID` to the function.
:::
```tsx
const id = '..,';
...
```tsx
type ChatMessage =  { type: 'text', message: string }
