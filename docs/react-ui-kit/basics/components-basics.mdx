...
`meeting.self` and participant objects from `meeting.participants` maps can be passed
...
While meeting state is stored in core SDK's classes, there are also some pure UI states that might change during a meeting.
For example, when user clicks the leave meeting button, there is a confirmation popup on the screen, this state reflects only a UI state.
