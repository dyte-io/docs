...
Dyte's UI Kit allows you to quickly modify all text visible in the video call.

Customizing the meeting locale involve the following steps:
...
```jsx
import { defaultLanguage } from '@dytesdk/react-ui-kit';
```
...
const useLanguage = (lang) => {
 ...
 <DyteMeeting meeting={meeting!} t={useLanguage(myLanguagePack)} />;
```

The `useLanguage` function takes in your custom locale object as an argument. It returns a function that returns the value corresponding to the key passed to it.

## Dyte's Default Locale Pack

Here is what Dyte's default locale pack looks like:
