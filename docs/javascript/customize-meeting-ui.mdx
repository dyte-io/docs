...
[`preset`](/getting-started#glossary) used to generate the `authToken`, you can
...
`element` to an HTMLElement, and whether the participant sees their own video
while in the waiting room using `showSelfVideo` (which is `true` by default).
...
[`roomMessage`](./advanced-usage#send-a-custom-message-to-other-participants-in-the-meeting-p2p-message--data-exchange),
to determine when to show an overlay, or you could build your own custom logic
around this.

:::info

Adding any overlay disables the context menu (right click) on the video
stream(s).
...
You can add overlay to the entire meeting grid, which is the area including all
the participant videos. Generally it will be all the area between the header and
the control bar.

Use `meeting.grid.setOverlay(HTMLElement)` to set overlay on top of the entire
