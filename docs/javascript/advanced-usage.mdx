...
function to the `onError` method of `DyteMeeting`. `onError` expects a function
which has a single param of type `Error` and is not expected to return anything.
...
`participant.setOverlay(HTMLElement)` where you pass an HTML string to the
function.
