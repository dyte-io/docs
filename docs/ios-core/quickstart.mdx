...
<<<<<<< HEAD 2. Add listeners and implement callback stubs ======= Add listeners
and implement callback stubs
...
v2, you just need to provide the participant's authToken. ======= Now you can
use `meeting` to join in the room. First step is to establish a connection to
the meeting server, you can do that with
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
| | `authToken` | After you've created the meeting, <br/> add each participant
to the meeting <br/> using the
[Add Participant API](https://docs.dyte.io/api?v=v2#/operations/add_participant)<br/>
(The presetName created earlier <br/> must be passed in the body <br/> of the
Add Participant API request) <br/> The API response contains the `authToken`. |
...
<<<<<<< HEAD For V2 APIs: ======= For V2 APIs: (Note: roomName is not required
for v2 APIs)
...
<<<<<<< HEAD Now, you have established the connection with the Dyte meeting
server successfully. Once `onMeetingInitCompleted()` is triggered, next step is
to join the room. ======= after `onMeetingInitCompleted()` Now to join in the
room, use
...
<<<<<<< HEAD callbacks for this action on meeting object if you have done
`meeting.addSelfEventsListener(selfEventsListener: self)`. ::: ======= callbacks
for this action on meeting object by registering observer. :::
