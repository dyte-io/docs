...
Here are some common values that can be used for the `UIBackgroundModes` key:

`audio`: This mode allows the app to play audio in the background.

`location`: This mode allows the app to receive location updates in the background.

`voip`: This mode allows the app to provide Voice over IP services.

`fetch`: This mode allows the app to download content in the background.

`remote-notification`: This mode allows the app to receive remote notifications in the background.

`newsstand-content`: This mode allows the app to download and process Newsstand content in the background.

`external-accessory`: This mode allows the app to communicate with external accessories in the background.
...
Sources:
Apple Developer Documentation: [Declaring Your App’s Supported Background Tasks](https://developer.apple.com/documentation/bundleresources/information_property_list/uibackgroundmodes)
