...
```swift
let meetingInfo = DyteMeetingInfo(roomName: MEETING_ROOM_NAME,
...
`baseUrl` can be found at [Dyte developer portal](https://dev.dyte.in/). You can
get the `roomName` and `authToken` using our backend APIs. Please refer
...
Similarly to leave the meeting you can call `leaveRoom()` on `meeting` object.
