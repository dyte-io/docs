...
`meeeting.participants.on()` method. Here are the supported events:
...
Trigger an event when a participant is removed.

```ts
const kickParticipants = useDyteSelector(
  (meeting) => meeting.participants.kick
);
```
...
Trigger an event when a participant starts / stops audio.

```ts
const { audioEnabled, audioTrack } = useDyteSelector((m) =>
...
Trigger an event when participant joins a room.

```ts
const participants = useDyteSelector((meeting) => meeting.self.roomJoined);
```
