...
You need to pass a [`DyteMeetingConfig`](./reference/connection-config) to [`DyteMeetingView`](./reference/dyte-meeting) to define which meeting should the client connect to and a couple of other setup parameters, out of which only `authToken`, `roomName`, `width` and `height` are mandatory.
