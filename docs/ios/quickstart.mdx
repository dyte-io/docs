...
Set the initialization properties in the `SetupViewController` class. You just
need to provide the participant's `authToken`.
...
```swift
//BASE_URL = "https://api.cluster.dyte.in"
let controller = SetupViewController(authToken: Constants.AUTH_TOKEN, enableAudio: true, enableVideo: true, baseUrl: Constants.BASE_URL, completion: { [unowned self] in
