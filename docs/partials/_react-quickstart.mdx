...
You need to import the `DyteProvider` from the `@dytesdk/react-web-core`. `DyteProvider` basically is a hook wrapper on `dytesdk/web-core`. This provides a meeting object to child components.
