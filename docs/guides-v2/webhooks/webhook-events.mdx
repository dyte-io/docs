...
This is a list of all the types of events we currently send. We may add more in
the future, so in developing and maintaining your code, you should not assume
that only these types exist.

## Types of events

You'll notice that all events follow a pattern: `resource.event`. Our goal is to
design a consistent system that makes things easier to anticipate and code
against.

Events are subscription-only; i.e: you need to opt-in to receive them. You will
not receive updates for event types that you have not opted-in to receive.
...
:::info Note that the `reason` can be one of `HOST_ENDED_MEETING` or
