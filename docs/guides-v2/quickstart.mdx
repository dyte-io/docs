...
principles. It has predictable resource-oriented URLs, accepts JSON-encoded
...
For the complete list of routes, see our [API documentation](/api/?v=v2).

### Step 2: Authenticate using your API Key

Dyte's APIs use API Keys to authenticate requests via HTTP Basic Auth. To
...
Create presets using the Developer portal. A preset is a set of permissions and
UI configurations that is applied to a participant.
...
[`recording.statusUpdate`](/guides-v2/webhooks) events using webhooks.
