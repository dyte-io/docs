...
```bash npm2yarn
npm install react-native-webrtc react-native-get-random-values react-native-url-polyfill react-native-document-picker react-native-file-viewer react-native-fs react-native-safe-area-context react-native-sound-player react-native-svg react-native-webview
```
