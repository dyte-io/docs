...
You can listen to livestream events by calling `addLivestreamEventsListener` on `dyteMobileClient` object, where `dyteMobileClient` is an instance of `DyteMobileClient()`.
...
### onLiveStreamStateUpdate
  ...
  The updated livestream state can be accessed via the DyteLivestreamData object using:
