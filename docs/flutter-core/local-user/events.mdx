...
-  Implement `DyteSelfEventsListener` to a class say `LocalUserListener`, 
and listen to event from `DyteSelfEventsListener` by overriding them. 
...
- Now to subscribe, pass the instance of `LocalUserListener` class created above to
  `addSelfEventsListener()` method of `dyteClient` instance as follows:
