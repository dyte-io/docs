...
The meeting chat object is stored in `dyteClient.chat`, which has
methods for sending and receiving messages. There are 3 types of messages that
can be sent in chat - `text`, `images`, and `files`.

The `dyteClient.chat.messages` array contains all the messages that
