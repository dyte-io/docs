...
The meetings polls object can be accessed using `ref.watch(dyteClient).polls`. It provides
methods to create polls, vote, and more.

`ref.watch(dyteClient).polls.polls` returns an array of all polls created in a meeting, where
