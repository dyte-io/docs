...
The `ref.watch(dyteClient).polls.vote()` method can be used to register a vote
on a poll. It accepts the following params.
...
```dart
final poll = ref.watch(dyteClient).polls.polls[0];
final selectedPollOption = poll.options[0];
meeting.polls.vote(poll, selectedPollOption);
```
