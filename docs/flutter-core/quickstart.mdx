...
You can also clone and run a sample application from the Flutter Core SDK GitHub
repository.
...
_Note: If you intend to publish your app to the Google Play or App Store, you'll
need to perform a few additional steps. So if you’re working on release builds
and not debug builds, refer to Release Builds._
...
Set the properties in the `DyteMeetingInfo` class. You need to provide the
meeting's `roomName` and the participant's `authToken`. Optionally you can also
add `baseUrl`.

| Name                      | Description                                                                                                                                                                                                                                                                                                                              |
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `baseUrl`<br/> (optional) | The question that is to be voted for.<br/> You can set to any of the following: <br/>(Default) [https://api.cluster.dyte.in](https://api.cluster.dyte.in) <br/> [https://api.staging.dyte.in](https://api.staging.dyte.in)                                                                                                               |
| `roomName` or `meetingID` | - for v1 APIs: pass the [roomName](https://docs.dyte.io/api/?v=v1#/operations/get_all_meetings)<br/> - for v2 APIs: pass the [meetingID](https://docs.dyte.io/api/?v=v2#/operations/create_meeting)                                                                                                                                      |
| `authToken`               | After you've created the meeting, <br/> add each participant to the meeting <br/> using the [Add Participant API](https://docs.dyte.io/api#/operations/add_participant)<br/> (The presetName created earlier <br/> must be passed in the body <br/> of the Add Participant API request) <br/> The API response contains the `authToken`. |
...
Here is a sample code for using Dyte with the Flutter application.
...
Add the following to your `android/app/build.gradle` to import the proguard
configuration.
...
Before getting started with iOS release builds, complete the following
prerequisites.
...
#### Perform the following steps for the iOS release builds:
...
3. Disable `BITCODE` in your Podfile.
