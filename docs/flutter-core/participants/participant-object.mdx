...
-  Implement `DyteParticipantEventsListener` to a class say `ParticipantEventListener`, 
and listen to event from `DyteParticipantEventsListener` by overriding them. 
This class can be user to manage your state in the application.
...
- Now to subscribe, pass the instance of `DyteParticipantEventsListener` class created above to
  `addParticipantEventsListener()` method of `dyteClient` instance as follows:
