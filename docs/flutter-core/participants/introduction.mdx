...
`DyteGridPagesInfo` object is designed to help you with pagination decisions. 
It is returned with `onGridUpdated` event.
...
The `setPage(int pageNumber)` method allows you to switch between 
pages of participants present in the meeting.
