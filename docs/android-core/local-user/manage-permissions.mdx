...
```kotlin
    val isCameraPermissionGranted = meeting.localUser.isCameraPermissionGranted
    val isMicrophonePermissionGranted = meeting.localUser.isMicrophonePermissionGranted
```

Similarely to listen to callbacks regarding media permissions once user joins
the meeting
