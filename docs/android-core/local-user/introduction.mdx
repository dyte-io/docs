...
To show localUser preview inside a view. use `getVideoView()` method on localUser. This method returns a View which can be added in any ViewGroup in android.
...
reflect across all participants ONLY if the change happens before `joinRoom()` the
meeting and after `init()`.
...
```kotlin
// switch camera
meeting.localUser.switchCamera(videoDevice: DyteVideoDevice)
```