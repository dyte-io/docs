...
The local user has the methods and properties on the local user media controls. Accessible via `self` within the `meeting` object.
...
```kotlin
meeting.self.setDisplayName("New Name")
```
