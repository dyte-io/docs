...
Plugins are webviews which can be added in any view group in android. To be able to get webview from DytePlugin one needs to first acivate a plugin. To do that all we need to do is call `plugin.active()` which will trigger a callback in `onPluginActivated()`. Similarely to deactivate a plugin one can call `plugin.deactivate()` and that plugin will be deactivated in the meeting.
