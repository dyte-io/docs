...
In the call, both hosts and participants have the ability to manage the stage. This allows for seamless control and organization of the session.
...
```kotlin
meeting.webinar.accessRequests
```
...
```kotlin
meeting.webinar.denyAccess(ids: List<String>)
```
