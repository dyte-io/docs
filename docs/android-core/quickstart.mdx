...
| `enableAudio` | You can control the state of audio track by passing `true` or `false` here. |     |
| `enableVideo` | You can control the state of video track by passing `true` or `false` here. |     |
...
To initialize the meeting, call the `init()` method obtained on
