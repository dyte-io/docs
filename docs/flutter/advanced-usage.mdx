...
The `DyteMeetingHandler` instance (referenced as `meeting` above) helps you
interact with the meeting. You can take actions on the meeting as well as the
...
`DyteMeetingHandler` type [here](./reference/meeting). Read on to understand how
to use these individual controls.
...
`List<DyteParticipant>` where all meeting participants are
[`DyteParticipant`](./reference/participant).
