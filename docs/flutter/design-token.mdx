...
Design tokens are the single source of truth to name and store design decisions. Design tokens are the design related values which are used to maintain a design system, which provides flexibility in customizing the overall design of a system with values such as, typography, spacing, colors, borders, and so on.
...
You can pass a single color to the `backgroundColor`, `brandColor` parameters.
...
To customize the border radius and border width of the UI Kit, you can use the `DyteBorderRadius` and `BorderWidth` enum values in the design token.
...
Here are the available `BorderWidth` values:
