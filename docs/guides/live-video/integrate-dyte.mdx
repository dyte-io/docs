...
On successfully creating a participant, you will recieve an authorization token (`authToken`) for each participant. Your server has complete control over when to distribute this token to the participants. Your client side needs the `authToken` to connect and interact with the meeting.
