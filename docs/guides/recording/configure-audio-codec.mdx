...
If [VP8](/guides/recording/configure-codecs) is selected for `videoConfig.codec`, changing `audioConfig.codec` is not allowed. In this case, the `audioCodec` is automatically set to `vorbis`.
