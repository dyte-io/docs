...
import RecordingStorage from '../../../static/static/guides/recording-storage.png';
...
3. Specify the details for your cloud provider. We support transferring
   recordings to AWS, Azure, and DigitalOcean storage buckets.

<img
  src={RecordingStorage}
  alt="Entering details for recording storage"
...
The `file_name_prefix` field allows you to modify the name of the recording file. The default format for recorded files is `roomname_timestamp`, but you can add an alphanumeric and underscore prefix to this format. For example, you could use the format `updaterecordingname_roomname_timestamp`.

For more informaation, see [start recording a meeting](https://docs.dyte.io/api#/operations/start_recording).
