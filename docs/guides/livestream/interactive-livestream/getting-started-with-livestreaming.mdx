...
This topic will guide you through the steps of getting started with interactive live streaming for your Dyte meetings, enabling you to broadcast and engage with your viewers in real time.

## Prerequisites

Before you begin the livestreaming process, it is recommended to familiarize yourself with the following topics:

- [Getting Started With Dyte](/getting-started)
- [Introduction to Dyte's Interactive Livestreaming](/guides/livestream/livestream-overview)

Creating an interactive livestreaming involves the following steps:
...
A preset is a set of permissions and UI configurations that are applied to hosts and participants. They determine the look, feel, and behavior of the livestream, with options to set varied permissions, such as the ability to create polls or chat, control access for plugins, and more. For more information on Presets, see [Add Presets](/guides/integrate-core-features/add-presets).
...
The preset created in the [step 2](/guides/livestream/interactive-livestream/getting-started-with-livestreaming#step-2-create-presets) must be passed in the body of the Add Participant API request. Specify the name of the preset in the `preset_name` parameter.
...
To start livestreaming a meeting, simply specify the meeting ID in the [Start livestreaming a meeting](/api#/paths/meetings-meeting_id--livestreams/post) API. You can also specify the video configurations for your stream, for example, height and width of the video.
If you wish to stop the streaming, use the [Stop livestreaming a meeting](/api#/operations/stop_livestreaming) API.
