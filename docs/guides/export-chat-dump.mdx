...
You can programmatically retrieve all chat messages of a Dyte session using -
  ...
  1. [Chat Replay API](/api/?v=v2#/operations/GetSessionChat).
  2. Setting up webhook for the [meeting.chatSynced](/guides/webhooks/webhook-events#meetingchatsynced) event

This guide walks you through the steps involved in exporting chat dump of a session using Chat Replay API. 
If you wish to use webhooks, see [Set Up Webhooks](/guides/webhooks/webhooks-and-events) to Receive Real-time Updates and [meeting.chatSynced](guides/webhooks/webhook-events#meetingchatsynced)
