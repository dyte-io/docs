...
You can also use [webhooks as described here](./subscribing-to-server-events#recordingstatusupdate) to subscribe to meeting status update events instead of polling.

## Recording states

Recording can have one of these states, which are also notified through [webhooks](./subscribing-to-server-events#recordingstatusupdate):
