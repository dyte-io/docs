...
4. Maximum recording time is 3 hours. Recording will automatically stop after the expiry of that period from the time of recording start. This option can be configured by passing the `maxSeconds` parameter in the start recording API.
