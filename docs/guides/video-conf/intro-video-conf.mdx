...
- **Breakout rooms**: Breakout rooms allow participants of a meeting to split into smaller groups for targeted discussions and collaboration. See [Create a Breakout Room](/guides/integrate-core-features/breakoutroom/create-breakout-rooms).
- **Virtual background**: You can add custom virtual background to Dyte video calls. See [Add Custom Virtual Background to Dyte meetings](/guides//advanced-features/add-virtual-background).
...
- **Recording**: Dyte records the audio and video of multiple users in a meeting, as well as interactions with Dyte plugins, in a single file using composite recording mode. See [Recording](/guides/integrate-core-features/recording).
