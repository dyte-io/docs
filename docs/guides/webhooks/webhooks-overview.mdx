...
The header provides information about the request, such as the type of data or the HTTP client. For each event that triggers the webhook, a header is included in the payload to describe the event.
...
| **Header**        | **Description**                                                                                                                                                                                                                                                                                                     |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dyte-webhook-id` | Webhook IDs are used to uniquely identify a specific webhook endpoint or events. See [Use Webhook IDs for Secure and Reliable Integrations](/guides/webhooks/identify-webhook-using-id).                                                                                                                            |
| `dyte-uuid`       | This is a unique UUID identifying the event.                                                                                                                                                                                                                                                                        |
| `dyte-signature`  | Dyte verifies the webhook events it sends to your endpoints by including a signature in the dyte-signature header of each event. This allows you to confirm that the event came from Dyte and not a malicious server impersonating Dyte. See [Verify Webhooks Using Dyte's Signature](/guides/webhooks/signatures). |
